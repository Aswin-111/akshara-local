...
//   // Inside your handleSidebarClick function, replace the polygon drawing code with this:
...
//       const newPolygons = [];
//       response.data.polygons.forEach((polygonData) => {
//         if (polygonData.coords && Array.isArray(polygonData.coords)) {
//           polygonData.coords.forEach((coordinateSet) => {
//             if (Array.isArray(coordinateSet) && coordinateSet.length > 0) {
//               const paths = coordinateSet.map((point) => {
//                 if (Array.isArray(point) && point.length === 2) {
//                   const [lng, lat] = point;
...
//                 // Optional: Fit bounds to show the polygon
...
export default function Home() {
    ...
    window.initMap = () => {
      ...
      mapInstance.addListener('click', async (event) => {
        ...
        try {
          ...
          setSideToggle(false);
          toast.error('Failed to get country information');
          console.error(err);
        }
      });
    };
    ...
    async function getLangData() {
      if (countryName) {
        ...
        try {
          ...
          const formattedNumber = formatter.format(response.data.languages.length || 0);
          setInfoToggle(false);
          setSidebarData(response.data.languages);
          setSideToggle(true);
          setTotalCount(formattedNumber);
          ...
          setSideToggle(false);
          toast.error(`Languages not found for country ${countryName}`);
          console.error(err);
        } finally {
          setIsLoading(false);
        }
  ...
  const handleSidebarClick = async (language) => {
    ...
    try {
      ...
      console.log(response.data)
      setToggle({ toggle: true, char: language });
      setCard(response.data.languageDetails);
      setInfoToggle(true);
      ...
      if (response.data.polygons && map) {
        ...
        response.data.polygons.forEach((polygonData) => {
          if (polygonData.coords && Array.isArray(polygonData.coords)) {
            const paths = polygonData.coords.map(coord => ({
  ...
  return (
    <div className="flex">
      <div><Toaster /></div>
      {sidebartoggle && (
        <div className="w-[20vw] h-[100vh] bg-white flex flex-col items-center relative">
          {isLoading && (
          ...
          {infotoggle === false ? (
                  ...
                  <div
                    ...
                    className={`w-full  py-3 mt-5 ml-5 border-2 text-[0.7rem] ${
                      toggle.toggle === true && toggle.char === i
                        ? 'text-white bg-[#5f9253] border-green-500'
                        : 'text-black border-slate-400'
                    } flex justify-center rounded-[0.7rem] cursor-pointer hover:bg-gray-50`}
                    key={index}
                    onClick={() => handleSidebarClick(i)}
                  >
                    {i}
            ...
            //       <button 
            //         className="w-[60%] bg-red-400 text-white py-3 rounded-xl mt-3 mb-4 hover:bg-red-500 transition-colors"
            //         onClick={handleClose}
            //       >
            //         Close
            //       </button>
                    ...
                    <div className="overflow-y-scroll overflow-x-hidden w-[16vw] h-[30vh] rounded-xl px-10 bg-white">
                      <div className='w-full'>
                        {cardData && Object.keys(cardData).length > 0 ? (
                          Object.entries(cardData).map(([key, value]) => (
                            <h2 key={key} className="text-black mt-3 text-sm">
                              {(value && !["id", "Sl_No", "createdAt", "updatedAt", "Script_Yes_No"].includes(String(key).trim())) 
                                ? `${key.replace(/_/g, ' ')} : ${value != null ? value.toString() : ""}`
                                : ""}
                            </h2>
                          ))
